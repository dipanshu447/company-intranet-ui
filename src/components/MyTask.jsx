...
export default function MyTask() {
    ...
    const tasksEle = task.map((t, index) => (
        <div className="flex gap-3 bg-[#f8fafa] dark:bg-[#263146] py-2 px-4 rounded-lg dark:text-white items-center justify-between" key={t.task}>
            <div className="flex gap-4">
                <input type="checkbox" onChange={() => toggleTask(index)} checked={t.done} />
                <div className={t.done ? 'line-through text-[#585858] dark:text-[#485c7e]' : ''}>{t.task}</div>
            </div>
            <span className="p-1 hover:bg-[#ececec] dark:hover:bg-[#303e58] rounded-2xl cursor-pointer" onClick={() => deleteTask(index)}>
    ...
    return (
        <div className="bg-white dark:bg-[#202a3b] shadow-md overflow-visible box-border flex flex-col p-6 gap-2 rounded-2xl flex-4 overflow-y-auto">
            <div className="flex justify-between items-center">
                <span className="text-lg text-[#323233] dark:text-white font-medium">My Tasks</span>
            ...
            <form className="flex justify-between mt-3 items-center dark:text-white gap-5" onSubmit={addTask}>
                ...
                <button className="py-2 px-4 bg-[#ececec] hover:bg-[#e4e4e4] dark:bg-[#2a374d] rounded-lg cursor-pointer">Add</button>
            </form>
